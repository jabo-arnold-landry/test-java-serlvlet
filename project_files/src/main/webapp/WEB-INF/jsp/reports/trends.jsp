...
<body>
            ...
            <div class="col-md-12">
                <div class="stat-card">
                    ...
                    <div style="position: relative; height: 150px; width: 100%;">
                        <canvas id="incidentHeatmap"></canvas>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Safety check to ensure Chart.js handles 1-point graphs properly
        if (labels.length === 1) {
            labels.unshift(''); labels.push('');
            var origLoad = loadData[0], origTemp = tempData[0], origInc = incidentData[0];
            loadData = [null, origLoad, null];
            tempData = [null, origTemp, null];
            incidentData = [null, origInc, null];
        }

        new Chart(document.getElementById('loadTrendChart'), {
            type: 'line',
            data: { labels: labels, datasets: [{ label: 'Avg Load %', data: loadData, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.3, spanGaps: true }] },
        ...
        new Chart(document.getElementById('tempTrendChart'), {
            ...
            data: { labels: labels, datasets: [
                { label: 'Avg Temp', data: tempData, borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', fill: true, tension: 0.3, spanGaps: true },
        ...
        new Chart(document.getElementById('incidentHeatmap'), {
            type: 'bar',
            data: { labels: labels, datasets: [{ label: 'Incidents', data: incidentData, backgroundColor: incidentData.map(function(v){return v===null?'transparent':(v>=3?'#ef4444':v>=1?'#f59e0b':'#10b981');}) }] },
