                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                                    ...
                                    <td>
                                        <div class="progress" style="height:20px;">
                                            <div class="progress-bar ${ups.loadPercentage > 80 ? 'bg-danger' : ups.loadPercentage > 60 ? 'bg-warning' : 'bg-success'}"
                                                style="width:${ups.loadPercentage != null ? ups.loadPercentage : 0}%">
                                                ${ups.loadPercentage}%</div>
                                        </div>
                                    </td>
                                    <td><span
                                            class="badge ${ups.status == 'ACTIVE' ? 'bg-success' : ups.status == 'FAULTY' ? 'bg-danger' : 'bg-warning'}">${ups.status}</span>
                                    </td>
                                    <td>${ups.locationRoom}</td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/ups/view/${ups.upsId}"
                                            class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i></a>
                                        <a href="${pageContext.request.contextPath}/ups/edit/${ups.upsId}"
                                            class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a>
                                        <a href="${pageContext.request.contextPath}/ups/delete/${ups.upsId}"
                                            class="btn btn-sm btn-outline-danger"
                                            onclick="return confirm('Are you sure you want to delete this UPS?')"><i
                            ...
                            <c:if test="${empty upsList}">
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">No UPS systems registered yet.
                                        Click "Add UPS" to get started.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
